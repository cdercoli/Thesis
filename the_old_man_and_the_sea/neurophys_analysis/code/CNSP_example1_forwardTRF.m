...
bandpassFilterRange = [0.5, 8]; % Hz (indicate 0 to avoid running the low-pass or high-pass filters or both)
...
for sub = 1:nSubs
    ...
    if shuffle_flag 
        stim = shuffleFeatures(stim, 14, 5, 'seed',sub);
    end
    ...
    stimFeature = cndNormaliseChiara(stimFeature);
    ...
    [stats,t] = mTRFcrossval_multimetric(stimFeature.data,neural.data,neural.fs,dirTRF,tmin,tmax,...
        lambdas,'verbose',0,'onsetVecDim',1,'onsetVecWinSt',300,'onsetVecWinEnd',500,'respGroundTruth',neural.data);
...
if shuffle_flag
    save([dataMainFolder,resultsFolder,'Brod_CLIP_results'],'rAllElec','rAllElecTC','rAllElecGT_TC','rAllElecGT','modelAll')
else
    save([dataMainFolder,resultsFolder,'BERT_only_results'],'rAllElec','rAllElecTC','rAllElecGT_TC','rAllElecGT','modelAll')
end
