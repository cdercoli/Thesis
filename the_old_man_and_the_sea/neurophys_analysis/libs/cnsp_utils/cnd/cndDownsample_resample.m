function cnd = cndDownsample_resample(cnd,downFs)
...
%     end
    ...
    cnd.data = cellfun(@(x) resample(x,downFs,cnd.fs,0),cnd.data,'UniformOutput',false);
    
    % Downsampling each trial/run of the external channels
    if isfield(cnd,'extChan')
        for extIdx = 1:length(cnd.extChan)
            cnd.extChan{extIdx}.data = cellfun(@(x) resample(x,downFs,cnd.fs,0),cnd.extChan{extIdx}.data,'UniformOutput',false);
        end
    end
